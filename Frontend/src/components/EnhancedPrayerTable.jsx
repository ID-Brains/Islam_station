...
const EnhancedPrayerTable = ({
    ...
    const onNextPrayerChangeRef = useRef(onNextPrayerChange);

    // Update the ref when onNextPrayerChange changes
    useEffect(() => {
        onNextPrayerChangeRef.current = onNextPrayerChange;
    }, [onNextPrayerChange]);
            ...
            try {
                ...
                const baseTimes = {
                    ...
                    isha: calculateTime(
                        19,
                        45 +
                            (methodConfig.isha === "90 min after maghrib"
                                ? 1.5
                                : methodConfig.isha / 10),
                        dayOfYear,
                    ),
                };
    ...
    const convertTo12Hour = useCallback(
        (time24) => {
                ...
                const prayerList = [
                    {
                        ...
                        time: prayerTimes.imsak
                            ? convertTo12Hour(prayerTimes.imsak)
                            : calculateTime(5, 20),
                    },
                    {
                        name: "Fajr",
                        time: prayerTimes.fajr
                            ? convertTo12Hour(prayerTimes.fajr)
                            : calculateTime(5, 30),
                    },
                    {
                        name: "Sunrise",
                        time: prayerTimes.sunrise
                            ? convertTo12Hour(prayerTimes.sunrise)
                            : calculateTime(6, 45),
                    },
                    {
                        name: "Dhuhr",
                        time: prayerTimes.dhuhr
                            ? convertTo12Hour(prayerTimes.dhuhr)
                            : calculateTime(12, 15),
                    },
                    {
                        name: "Asr",
                        time: prayerTimes.asr
                            ? convertTo12Hour(prayerTimes.asr)
                            : calculateTime(15, 45),
                    },
                    {
                        name: "Maghrib",
                        time: prayerTimes.maghrib
                            ? convertTo12Hour(prayerTimes.maghrib)
                            : calculateTime(18, 20),
                    },
                    {
                        name: "Isha",
                        time: prayerTimes.isha
                            ? convertTo12Hour(prayerTimes.isha)
                            : calculateTime(19, 45),
                    },
                ];
