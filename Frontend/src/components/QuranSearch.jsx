  ...
  <div className="bg-base-100 rounded-xl p-6 border border-white/10 hover:border-primary/30 transition-all duration-300">
    ...
    <div className="space-y-4">
      <div className="text-right">
        <p className="text-2xl font-arabic leading-relaxed text-base-content mb-3" dir="rtl">
          {result.arabic_text}
        </p>
      </div>
...
const QuranSearch = () => {
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/quran/search.json?${params}`);
          ...
          {searchResults.length > 0 ? (
            <div className="space-y-4">
              {searchResults.map((result) => (
                <SearchResultCard key={`${result.surah_id}-${result.verse_number}`} result={result} />
              ))}
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-base-content mb-2">No results found</h3>
